const state={category:'add-subtract',count:20,index:0,score:0,questions:[],mistakes:[],startedAt:0,timerId:null,elapsed:0};
...
function submit(event){event.preventDefault();const input=$('#answerInput');if(input.value.trim()==='')return;const q=state.questions[state.index],answer=Number(input.value),feedback=$('#feedback');if(answer===q.answer){state.score++;feedback.textContent=`回答正确！继续保持 ✦${q.tip?` · ${q.tip}`:''}`;feedback.classList.add('good')}else{feedback.textContent=`正确答案是 ${q.answer}${q.tip?` · 技巧：${q.tip}`:''}`;feedback.classList.add('bad');state.mistakes.push({q,answer})}input.disabled=true;setTimeout(()=>{input.disabled=false;if(state.index<state.count-1){state.index++;showQuestion()}else finish()},700)}
...
function goHome(){clearInterval(state.timerId);result.classList.add('hidden');quiz.classList.add('hidden');setup.classList.remove('hidden')}
$('#startButton').addEventListener('click',start);$('#answerForm').addEventListener('submit',submit);$('#quitButton').addEventListener('click',goHome);$('#retryButton').addEventListener('click',start);$('#homeButton').addEventListener('click',goHome);loadHistory();
...
function showQuestion(){const q=state.questions[state.index];$('#progressText').textContent='第 '+(state.index+1)+' / '+state.count+' 题';$('#progressBar').style.width=(state.index/state.count*100)+'%';$('#questionText').textContent=q.text;$('#feedback').textContent='';$('#feedback').className='feedback';state.acceptingAnswer=false;setVoiceButton(false);setVoiceStatus('点击话筒后，直接说出答案。')}
...
$('#startButton').addEventListener('click',start);$('#voiceButton').addEventListener('click',startListening);$('#quitButton').addEventListener('click',goHome);$('#retryButton').addEventListener('click',start);$('#homeButton').addEventListener('click',goHome);loadHistory();
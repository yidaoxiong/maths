...
export async function onRequestPost({ request, env }) {
  ...
  if (!body || !isValidClientId(body.clientId) || !/^\d{4}-\d{2}-\d{2}$/.test(body.date)) return json({ error: 'Invalid session.' }, 400);
  ...
  await env.DB.prepare(`
    INSERT INTO practice_sessions (client_id, practice_date, total_questions, correct_answers, elapsed_seconds, mistake_count)
    VALUES (?, ?, ?, ?, ?, ?)
  `).bind(body.clientId, body.date, total, correct, elapsed, mistakes).run();
